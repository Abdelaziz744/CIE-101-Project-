...
#include "Game.h"
#include "../Config/GameConfig.h"
#include "../UI/Warehouse.h"
#include "../Entities/Animal.h"
#include "../UI/BudgetBar.h"
#include <ctime>
#include <iostream>
using namespace std;
void Game::createFoodAreas()
{
	int startY = config.toolBarHeight * 2 + 40;
	...
	for (int i = 0; i < 3; i++)
	{
		...
		p.x = positions[i][0];
		p.y = positions[i][1];
		foodAreas[numFoodAreas] = new FoodArea(this, p, 100, 60, positions[i][2]);
		foodAreas[numFoodAreas]->draw();
		numFoodAreas++;
	}
}
...
void Game::gameTick()
{
	// Decrease food areas once per tick
	for (int i = 0; i < numFoodAreas; i++)
		if (foodAreas[i] && foodAreas[i]->isActive)
			foodAreas[i]->decreaseFood();

	// Tick chick production
	if (gameBudgetbar)
		gameBudgetbar->tickChicks(warehouse);
...
void Game::go()
{
	...
	while (!isExit)
	{
		string budget_string = "BUDGET = $" + to_string(budget);
		printBudget(budget_string);
		printMessage("Click an animal icon to buy it!");
		...
		// Tick once per second based on real time
